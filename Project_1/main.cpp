...
TEST(LongKey) {
CHECK_EQUAL("БЦДЕФ",modAlphaCipher("БЦДЕФГШИЖК").encrypt("
ААААА"));
}
TEST(LowCaseKey) {
CHECK_EQUAL("БЦДБЦ",modAlphaCipher("бцд").encrypt("ААААА")
...
TEST_FIXTURE(KeyB_fixture, LowCaseString) {
CHECK_THROW(p-
>decrypt("рёУЭЛБГЬЩЁМРПДФМАУЭ"),cipher_error);
}
...
CHECK_THROW(p-
>decrypt("ЛЙСЙММВСЙМ2019ТАГСБИ"),cipher_error);
}
TEST_FIXTURE(KeyB_fixture, PunctString) {
CHECK_THROW(p-
>decrypt("РЁУЭЛБ,ГЬЩЁМРПДФМАУЭ"),cipher_error);
}
TEST_FIXTURE(KeyB_fixture, EmptyString) {
